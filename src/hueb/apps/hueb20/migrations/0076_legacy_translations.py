...
def load_legacy_translations(apps, schema_editor):
    OrigAssign = apps.get_model("hueb_legacy", "OrigAssign")
    ...
    for translation in Translations.objects.all():
        ...
        document.translation_ref_legacy = translation
        document.app = LEGACY
        ...
        if translation.ddc is not None:
            if DDC.objects.filter(ddc_number=translation.ddc.ddc_number).exists():
                ...
                raise Exception(
                    "There is no ddc assigned to " + translation.ddc.ddc_name
                )
            ...
            for orig_assign in OrigAssign.objects.filter(translation=translation).all():
                if (
                    ...
                    and Document.objects.filter(
                        original_ref_legacy=orig_assign.original
                    ).exists()
                ):
                ...
                if publi != "":
                    if not Person.objects.filter(name__iexact=publi).exists():
                        if "unbekannt" not in publi.lower():
                            person = Person()
                            person.name = publi
                            person.publisherTranslation_ref_legacy = translation
                            person.app = LEGACY
                            person.save()
...
def unload_legacy_translations(apps, schema_editor):
    Document = apps.get_model("hueb20", "Document")
...
class Migration(migrations.Migration):
    dependencies = [
        ("hueb20", "0075_legacy_originals"),
        # ("hueb_legacy_lidos", "0009_alter_original_manual_keys"),
    ]

    operations = [
        migrations.RunPython(load_legacy_translations, unload_legacy_translations)
    ]
