    ...
    for file in [
        ...
        with open(file) as f:
            ...
            for row in reader:
                ...
                if not Document.objects.filter(title=translation).exists():
                    ...
                    if language_trans == "?":
                        language_trans = None
                    if language_trans:
                        if Language.objects.filter(language_de=language_trans).exists():
                ...
                if (
                    ...
                    doc = Document.objects.get(title=translation)
                ...
                for index in [1, 8, 9, 19, 20]:
                    ...
                    if comment:
                        ...
                        new_comment.document = Document.objects.get(title=translation)
                        new_comment.save()
                ...
                new_filing.document = Document.objects.get(title=translation)
                try:
                    new_filing.save()
                except Exception:
                ...
                # original document
                if not Document.objects.filter(title=title_org).exists():
                    ...
                    if language_org:
                        if Language.objects.filter(language_de=language_org).exists():
                ...
                if (
                    ...
                    doc = Document.objects.get(title=title_org)
                    doc.main_author = Person.objects.get(
                ...
                if (
                    any([author_inter, year_inter, title_inter, language_inter])
                    and not Document.objects.filter(title=title_inter).exists()
                ):
                    ...
                    if language_inter:
                        if Language.objects.filter(language_de=language_inter).exists():
                    ...
                    if (
                        ...
                        doc = Document.objects.get(title=title_inter)
                        doc.main_author = Person.objects.get(
                            name=sorted(
...
class Migration(migrations.Migration):
    dependencies = [("hueb20", "0127_hues_import_1")]
