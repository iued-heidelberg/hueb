...
def load_translation_translator(apps, schema_editor):
    Legacy_TranslationTranslator = apps.get_model("hueb_legacy_latein", "TranslationNewTranslatorNew")
    ...
    for legacy_translation_translator in Legacy_TranslationTranslator.objects.all():
        if Contribution.objects.filter(translationTranslator_ref=legacy_translation_translator).exists():
            continue
        ...
        try:
            new_contribution.person = Person.objects.get(translator_ref=legacy_translation_translator.translator)
        except Exception:
            try:
                new_contribution.person = Person.objects.get(name__iexact=legacy_translation_translator.translator.name)
            except Exception:
                print("Could not find person " + legacy_translation_translator.translator.name)
                continue
        new_contribution.document = Document.objects.get(translation_ref=legacy_translation_translator.translation)
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(load_translation_translator, unload_translation_translator)]
