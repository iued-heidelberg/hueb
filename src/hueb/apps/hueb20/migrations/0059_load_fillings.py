    ...
    for legacy_loc_assign in Legacy_LocAssign.objects.all():
        ...
        if Archive.objects.filter(location_ref=legacy_loc_assign.loc_new).exists():
            new_filing.archive = Archive.objects.get(
                location_ref=legacy_loc_assign.loc_new
            )
        elif legacy_loc_assign.loc_new is not None:
            raise Exception(
                "There is no archive assigned to " + legacy_loc_assign.loc_new.id
            )

        new_filing.signatur = legacy_loc_assign.signatur

        assert (legacy_loc_assign.orig_new is not None) or (
            legacy_loc_assign.trans_new is not None
        )

        if (
            legacy_loc_assign.orig_new is not None
            and legacy_loc_assign.orig_new.title != ""
            and legacy_loc_assign.orig_new.title is not None
        ):
            ...
            legacy_loc_assign.trans_new is not None
            and legacy_loc_assign.trans_new.title != ""
            and legacy_loc_assign.trans_new.title is not None
        ):
